...
/*
pub fn test (objA: &mut particle, objB:&mut particle)
{
	objA.set_samp(objA.get_curr());
	objB.set_samp(objB.get_curr());
    let t1:&dyn Any = objA.as_any();
	let t2:&dyn Any = objB.as_any();
	test_types(t1, t2);	
}
*/
/*
fn test_types (objA: & Any, objB:& Any)->bool
{
	match objA.downcast_ref::<polygon_particle>() 
	{
        Some(x) => 
		{
			//println!("it was polygon_particle: {}!", x);
			return test_types_poly_vs(&mut x, objB);
		},
        None => unreachable!(),
    }
	return false;
...
*/
/*
fn test_types_poly_vs(objA:&mut polygon_particle, objB:& Any)->bool
{
	match objB.downcast_ref::<polygon_particle>() 
	{
        Some(x) => 
		{
			//println!("it was polygon_particle: {}!", x);
			return testPolygonvsPolygon(objA, &mut x);
		},
        None => unreachable!(),
    }
	return false;
